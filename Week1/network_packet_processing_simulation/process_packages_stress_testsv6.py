...
class Buffer:
	...
	def Process(self, request):
		...
		if len(buffer1.finish_time) == self.size:	
			...
			previous_process_time = buffer1.process_time.pop()
			previous_finish_time = buffer1.finish_time.pop()
			if request.process_time == 0 and previous_process_time == 0: 	
			...
			elif previous_finish_time - previous_process_time >= request.arrival_time:	
        		...
        		#else insert request into buffer and return process with arrival time
			...
			previous_finish_time = buffer1.finish_time.pop()
			buffer1.process_time.insert(0,request.process_time)
			if request.arrival_time > previous_finish_time:
				buffer1.finish_time.insert(0, request.arrival_time + request.process_time)
			else:
				buffer1.finish_time.insert(0, request.arrival_time + request.process_time + previous_finish_time)
			return Response(False, max(request.arrival_time, previous_finish_time))
